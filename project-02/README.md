...
* **브랜드명**: LITHOS(리토스)
* **브랜드명의 의미**: LITHOS는 그리스어로 ‘돌(Stone)’ 또는 ‘암석’을 뜻한다. 오랜 시간 비바람을 견디며 자리를 지키는 암석처럼, 다양한 환경에서 스마트폰을 안정적으로 지지한다는 브랜드 철학을 담고 있다. 돌이 가진 무채색과 단단한 이미지는 고프코어의 어스 톤 미학 및 제품의 견고한 금속 소재와도 잘 어울린다.
* **브랜드 정체성**: 일상과 아웃도어의 경계에서 활동하는 사람들을 위한 액티브 테크기어 브랜드이다. 사용자가 장소와 환경에 구애받지 않고 자신의 일상과 움직임을 스마트폰으로 안정적으로 기록할 수 있도록 돕는다.
* **선정 타겟**: 스마트폰으로 일상을 기록하는 것이 자연스러운 20~30대 고프코어룩 선호자와 일상·운동·아웃도어 활동의 경계를 자유롭게 넘나드는 액티브 웰니스족 및 콘텐츠 크리에이터
* **톤앤매너**:
  * **Rugged yet Refined(거칠지만 정제된)** — 암석과 자연환경에서 연상되는 견고함에 세련된 금속 소재의 감성을 결합한다.
  * **Solid and Confident(단단하고 자신감 있는)** — 과장된 표현보다 제품의 안정적인 고정력과 활용성을 감각적인 장면으로 전달한다.
* **핵심 차별점(USP)**:
  1. 카라비너 구조를 활용해 주머니가 없어도 배낭이나 의류 웹빙 등에 간편하게 결합할 수 있는 **‘포켓리스(Pocketless) 기동성’**
  2. N52 자석과 폴딩 관절을 활용하여 아웃도어, 운동 및 일상 공간에서 다양한 방식으로 스마트폰을 고정할 수 있는 **‘올터레인(All-Terrain) 마운트’** 기능
  3. 접어서 간편하게 휴대하고 필요할 때 즉시 펼쳐 사용할 수 있는 **멀티유즈 폴딩 구조**
* **광고의 목적**: 브랜드 인지도 형성과 구매 전환을 목적으로 한다. 거친 아웃도어 환경에서도 활용할 수 있는 견고한 기능성과 카페·사무공간 등 일상에서도 자연스럽게 어울리는 세련된 범용성을 함께 보여줌으로써 제품에 대한 관심과 구매 욕구를 높인다.
* **캠페인 콘셉트**: 도시와 자연의 경계를 허무는 단단한 기록 도구. 거친 숲과 따뜻한 카페라는 대조적인 공간을 빠르게 전환하여, 하나의 제품이 아웃도어 장비와 일상용 스마트폰 액세서리의 역할을 모두 수행한다는 점을 전달한다.
...
* **영상 길이**: 10초
* **서사 구조**: 대조를 통한 기승전결 (거친 환경 제시 → 일상으로의 극적 전환 → 완벽한 휴대성 제안)
...
* **이미지 생성 도구 (Nano Banana 2, Flow)**: 
  ...
  * 고프코어 무드의 거칠고 사실적인 자연환경과 제품의 메탈 질감, 빛 반사를 가장 하이엔드 상업 사진처럼 디테일하게 구현하기 위해 선택함.
* **비디오 생성 도구 (FLOW)**: 
...
* **프롬프트 및 AI 활용 전략**:
  * 네이토에 있는 지피티, 클로드, 제미나이를 이용해 프롬프트 작성은 가능하지만 이미지 생성이 극히 제한되어 있어 유료 결제로 사용하고 있는 Gemini(Flow)를 위주로 이미지와 영상을 생성.
  * ChatGPT를 대안으로 준비.
  * TTS 생성은 구글 AI 스튜디오를 이용 (영상에는 사용하지 않음).
* **일관성 (캐릭터/제품) 유지 팁**: 
  * 플로우 캐릭터 고정 기능 이용.
